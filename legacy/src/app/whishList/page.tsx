...
export default function WishList() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get('http://localhost:3000/wish/getAll');
  ...
  const deleted = async (idWishlist: number) => {
    try {
      await axios.delete(`http://localhost:3000/wish/delete/${idWishlist}`);
      console.log('deleted fav');
      setWishes((prevWishes) => prevWishes.filter((wish) => wish.idWishList !== idWishlist));
    } catch (error) {
      console.error(error);
    }
  };
